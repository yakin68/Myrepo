...
## 1.yol grep serdar event_history.csv | grep TerminateInstances | grep -Eo "i-[a-zA-Z0-9]{17}" | sort | uniq > ikinciresult.txt 

## 2.yol cat event_history.csv | grep serdar | grep TerminateInstances | tee result1.txt | awk -F ',' '/i-/ {print $1 $10}' result1.txt | cut -d '"' -f 1,7 | tee yak1.txt
## 3.yol awk -F "," ' $1=="serdar" && $4=="TerminateInstances" ' event_history.csv > result.txt
## 4.yol awk -F, '{ if ($1 == "serdar" && $4 == "TerminateInstances") print $0;}' event_history.csv | grep -o "i-[a-zA-Z0-9]*" > result.txt
...
## */1 * * * * sudo /home/ec2-user/Project-101/Third_Part/backup.sh ## enson bu komut
