...
```bash
aws ec2 describe-security-groups --group-names roman_numbers_sec_grp  # sorgulatma varmı yokmu
```
...
pip3 install flask
cd /home/ec2-user
FOLDER="https://raw.githubusercontent.com/yakin68/Myrepo/main/Project-001-Roman-Numerals-Converter/"
wget ${FOLDER}/app.py
...
```
```bash
aws ec2 run-instances --image-id $LATEST_AMI --count 1 --instance-type t2.micro --key-name end-key-pair --security-group-ids sg-05c19d0ccaacdd3df --tag-specifications 'ResourceType=instance,Tags=[{Key=Name,Value=roman_numbers}]' --user-data file:///home/ec2-user/userdata.sh
or
...
aws ec2 run-instances \
    ...
    --security-group-ids sg-05c19d0ccaacdd3df \
...
```bash
aws ec2 describe-instances --filters "Name=tag:Name,Values=roman_numbers"  ## EC2 örneklerini tanımlayan bilgileri almak için 
```

- You can run the query to find Public IP and instance_id of instances:
```bash
aws ec2 describe-instances --filters "Name=tag:Name,Values=roman_numbers" --query 'Reservations[].Instances[].PublicIpAddress[]'  ## filitreleme yaparak puplic Ip bakmak

aws ec2 describe-instances --filters "Name=tag:Name,Values=roman_numbers" --query 'Reservations[].Instances[].InstanceId[]'     ## filitreleme yaparak instanceId bakmak
```
